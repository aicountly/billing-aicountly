...
/**
 * The bank withdrawals this product recorded, most recent first.
 *
 * WHAT THIS IS, PRECISELY, because the distinction matters: it is the
 * withdrawal REQUESTS Billing made and Books accepted, each carrying the
 * voucher reference Books gave back. It is not a second cash book and does not
 * claim to be every withdrawal in the accounts — a contra entered directly in
 * Smart Books is not here, and the screen says so rather than quietly showing a
 * short list.
 *
 * That is why it is allowed to exist at all, on the same terms as
 * ExpenseHistory: the request row already exists (it is what makes a retry
 * safe), and the only thing this adds is reading the last few of them back.
 * Every NAME on the screen — the bank, the cash account — is read live from
 * Books on this request, because those belong to Books and a copy of them here
 * would be the thing this architecture forbids.
 *
 ...
 */
final class BankWithdrawalHistory
{
    /** The window the withdrawal screen summarises beside the balance. */
    public const SUMMARY_DAYS = 30;

    private const BASIS = 'Withdrawals recorded from Billing in this company and year. '
        . 'Withdrawals entered directly in Smart Books are in Books\' own contra register.';

    /** The same caveat, said shorter, because it sits under one figure rather than a list. */
    private const SUMMARY_BASIS = 'Counted from the withdrawals recorded in Billing. '
    ...
    public function recent(int $limit = 4): array
    {
        ...
        $requests = Db::all(
            "SELECT request_id, transaction_date, payload, books_voucher_id, books_voucher_no, created_at
             ...
             WHERE {$scope} AND kind = 'bank_withdrawal' AND status = 'POSTED'
        ...
        foreach ($requests as $request) {
            ...
            $bankId = self::id($payload['from_account_id'] ?? null);
            $cashId = self::id($payload['to_account_id'] ?? null);
            ...
            $rows[] = [
                ...
                'reference_no'      => self::text($payload['instrument_no'] ?? $payload['reference_no'] ?? null),
    ...
    /**
     * What has been taken out of one bank account lately, and how often.
     *
     * Context beside the balance, not a ledger figure: it counts the withdrawals
    ...
    public function summary(int $bankAccountId, int $days = self::SUMMARY_DAYS): array
    {
        ...
        $row = Db::first(
            "SELECT COUNT(*) AS txn_count,
             ...
             FROM billing_transaction_requests
             WHERE {$scope} AND kind = 'bank_withdrawal' AND status = 'POSTED'
               AND payload->>'from_account_id' = :bank
