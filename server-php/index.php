...
/**
 * Billing API — front controller.
 *
 * Deployed to <document root>/api, so it is same-origin with the React app on
 * both billing.aicountly.com and billing.gh.aicountly.com.
 *
 * Routes:
 ...
 *   *    /api/v1/...          the Billing API proper — see src/Routes.php
 */
...
try {
    ...
    error_log('[billing] database error on ' . $path . ': ' . $e->getMessage());
    Http::error(503, 'database_unavailable', 'The Billing database is not reachable right now. Please retry.');
} catch (\Throwable $e) {
    error_log('[billing] unhandled error on ' . $path . ': ' . $e->getMessage() . ' @ ' . $e->getFile() . ':' . $e->getLine());
