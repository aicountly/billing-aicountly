...
check('filters narrow the list and the count beneath it agrees', function () use ($ctx, $auth) {
    ...
    // Counted from the unfiltered list rather than written down: the stub's
    // bills are shared with the receivables and payables suites and they move.
    // What must hold is that the filter and the count under it agree.
    $everyone = $directory->page(['side' => 'all', 'limit' => 50]);
    $owed = count(array_filter($everyone['rows'], static fn (array $row) => ($row['outstanding'] ?? 0) > 0));

    $owing = $directory->page(['side' => 'all', 'balance' => 'outstanding', 'limit' => 50]);
    assertTrue($owed > 0, 'somebody owes something, or this proves nothing');
    assertSame($owed, $owing['total'], 'the count under the list matches what the filter kept');
    foreach ($owing['rows'] as $row) {
        assertTrue(($row['outstanding'] ?? 0) > 0, $row['name'] . ' has an open balance');
    }
...
check('sorting by what is owed puts the largest debt first and the unknown last', function () use ($ctx, $auth) {
    ...
    // The rule, not the row: which party owes the most depends on fixtures the
    // receivables and payables suites share and change. What must hold is that
    // the amounts descend, and that a party whose balance could not be read
    // sorts LAST rather than leading as a nought would.
    $seen = null;
    $nullSeen = false;
    foreach ($page['rows'] as $row) {
        ...
        assertTrue(!$nullSeen, 'a known balance came after an unknown one');
        assertTrue($seen === null || $row['outstanding'] <= $seen, $row['name'] . ' is out of order');
        $seen = $row['outstanding'];
    }
    assertTrue($seen !== null, 'at least one balance was readable');
});
