# AICOUNTLY auth workflow (Billing)

How Billing signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `billing.aicountly.com` (or `billing.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/billing?returnUrl={origin}/auth/callback`.
...
| Billing host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `billing.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `billing.aicountly.com/api` |
| `billing.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `billing.gh.aicountly.com/api` |
...
`billing` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://billing.gh.aicountly.com/api/health
...
curl -i -X POST https://billing.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://billing.gh.aicountly.com/api/global/login   # expect 404
```
