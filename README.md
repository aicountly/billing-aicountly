...
`expense`, or `bank-withdrawal` — `?as=biller` narrows the profile, and
`?fail=recent,balance` makes those endpoints answer 503, which is how the "one
panel is down, the form still works" states get checked. `vite build` takes `index.html` only, so none of
