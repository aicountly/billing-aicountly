 ...
 * product's own API (`v1/manage/*`, read-only). React Native port of
 * web/src/services/manage.ts — no adaptation needed, this is a plain API
 * client call with no browser-specific code in it.
 *
 * Nothing here is cached or stored. The picker asks on the request that draws
 * it, which is the only way a list of companies cannot go stale when someone
 ...
 * single unbounded response is how that person's picker times out. MAX_PAGES
