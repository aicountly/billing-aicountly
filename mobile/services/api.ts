/**
 * Typed fetch wrapper for the Billing API — React Native port of
 * web/src/services/api.ts.
 *
 * What it encodes so screens do not have to:
 *
 ...
 *
 * Adapted from web: no `window`, so URLs are built from getApiBaseUrl()'s
 * already-absolute base directly (RN's URL has no second base-URL argument).
 * download()/upload() are dropped — native file export/import is a follow-up.
 */
